...
function replySub(rid,type) {
  ...
  $.ajax({
    ...
    url: '/comment/add',
    dataType: "json",
    data:{'type':type,'comcontent':$("#replycontent").val(),'cid':tocid,'rid':rid,'vcode':$('#vcode').val()},
    beforeSend:function(){
    ...
    success: function(result){
      if (result.code==1) {
        ...
        reloadComments(type,rid,curPage);
      }else{
        alert(result.msg);
      };
    },
...
function reloadComments(type,rid,page){
    $.get('/comment/commentList/'+type+'/'+rid+'/'+page,function(data,status){
            $('.comment').html(data);
        }
    );
}
